...
public class UpsertProfileImageCommandTests : CommandTestBase<UpsertProfileImageCommand>
{
    ...
    [Fact()]
    public Task Send_CommandWithEmptyImageData_ShouldThrowValidationError()
    {
        var sut = CreateSut();
        sut.ImageData = [];
        return Assert.ThrowsAsync<ValidationException>(() => Send(sut));
    }
    ...
    public async Task Send_WhenImageNotExistsIndDb_ShouldAddNewRecordToDbContext()
    {
        ...
        var result = await Send(sut).ConfigureAwait(true);
    ...
    {
        var sut = CreateSut();
        var seedImg = await GetDbContext().SeedImage().ConfigureAwait(true);
    ...
    private static void AssertPersonImage(PersonImage img, UpsertProfileImageCommand sut)
    {
        ...
        sut.ImageType.Should().Be(img.ImageType);
        sut.ImageData.Should().NotBeNullOrEmpty();
    }
    ...
    protected override UpsertProfileImageCommand CreateSut()
    {
        return new UpsertProfileImageCommand()
        {
            ...
            ImageType = IProfileImageService.DefaultImageMimeType
        };
    }
}